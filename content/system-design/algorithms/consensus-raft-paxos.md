...
Consensus is specifically designed to prevent split-brain (two nodes both believing they're Leader and accepting conflicting writes) - a correctly majority-gated protocol cannot produce two committed-but-conflicting values. What *does* happen under partition is a stale Leader on the minority side continuing to think it's Leader and serving reads from data that can no longer be updated - a **stale-leader read**, not split-brain. Guarding against it requires either routing reads through the same quorum check as writes (a "read index"), or a lease mechanism with a bounded clock-skew assumption - see [Linearizable Reads Without a Log Append](#linearizable-reads-without-a-log-append) below.

### Linearizable Reads Without a Log Append

A naive Leader could serve a read straight from its own local state machine, but that's only safe if the Leader is still actually the Leader - the stale-leader problem above. Appending every read to the log as a no-op entry would fix that (it forces the same majority round-trip a write gets) but pays a full disk-write and replication round-trip for something that never changes the log's content, which is wasteful at read-heavy workloads. Raft's **ReadIndex** protocol splits the difference: before serving a read, the Leader records the current commit index as `readIndex`, confirms it is still Leader by exchanging a heartbeat round with a majority of Followers (no log entry needed, just proof of continued leadership), then waits for its local state machine to apply up to `readIndex` before answering - this gets linearizability for the price of one network round-trip instead of a disk-durable log append. **Lease-based reads** go further: after a Leader has held a majority of heartbeat acknowledgments within a bounded lease interval, it assumes continued leadership for the remainder of that lease without a fresh round-trip per read, trading a round-trip for a clock-skew assumption - it's only safe if every node's clock drifts less than the lease bound, since a Leader that thinks its lease is still valid while a new Leader has already been elected elsewhere would serve a stale read with no protocol-level check to catch it.

⚖️ **Decision Framework** - ReadIndex costs one heartbeat round-trip per read but has no clock-safety assumption; a lease trades that round-trip away for lower read latency at the cost of trusting bounded clock skew across the cluster - pick ReadIndex when clocks can't be trusted (multi-region, virtualized hosts with clock drift) and a lease when read latency dominates and the deployment can bound skew (e.g. NTP-disciplined hosts in one datacenter).
