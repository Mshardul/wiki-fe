...
Suffix links let the construction jump directly between related insertion points instead of re-walking from the root for every suffix. The <abbr>amortized</abbr> argument: the "active point" used during construction can be shown to move a total of O(n) positions across the *entire* build (not per suffix) - the same style of accounting argument used for dynamic array doubling, just applied to tree-descent distance instead of array copies.
