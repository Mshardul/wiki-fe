...
**Base case.** Before the first step, the current position is `(MAX_LEVEL, head)` - the <abbr>sentinel</abbr> head node at the topmost level. No real node has been examined yet, so the invariant ("every node before the current position at every level ≤ current level is confirmed < target") holds **vacuously**: there is nothing before the head to falsify it.
