...
A **Union-Find** (also called **Disjoint-Set Union, DSU**) is a forest of rooted trees where each tree represents one set - the root serves as the canonical representative, and any node can ask "who is my root?" in near-constant <abbr>amortized</abbr> time, merging two trees into one just as fast.
...
> **Note:** This trace shows iterative path compression (two-pass). Recursive path compression on the same call would also compress node 2 directly to 0 - the recursive call on node 2's parent propagates the root all the way back up the <abbr>call stack</abbr>, setting every node's parent to the root on the return path.
...
The Tree/heap family's defining trait is a **maintained <abbr>invariant</abbr>** along root-to-node paths, with explicit repair operations that restore it after each mutation. In a binary search tree the invariant is an ordering; in a heap it's a priority relationship; in Union-Find the invariant is **root reachability and representative consistency**.

**Representations - array-of-parents vs explicit tree nodes.** Union-Find's forest is almost universally implemented as a **flat `parent[]` array** (index = node, value = that node's parent, or itself if root) rather than as explicit heap-allocated tree nodes with pointer fields. This is a deliberate representation choice, not an accident: (1) node identities are dense small integers (0..n-1) in the overwhelming majority of use cases, so array indexing is a direct O(1) lookup with zero pointer-chasing overhead; (2) the array is contiguous, so sequential and semi-random access during `find()` benefits from cache-line prefetching in a way a pointer-linked tree cannot; (3) path compression becomes a simple array write (`parent[x] = root`) instead of a pointer rewire through node objects. The one place an explicit-node representation appears is **string-keyed or object-keyed DSU** (see [Practice problem 3, Accounts Merge](#3-accounts-merge)), where the "array" becomes a `dict[str, str]` - same logical structure (a parent map), traded for hash-lookup overhead in exchange for non-integer keys. Both representations obey the identical forest invariant below; the array-vs-dict choice is purely about key-space density, not semantics.
