...
<!-- DS4 + U3: decision cues naming ≥1 sibling structure (e.g. segment-tree, sorted array with binary search) — PROSE. The scannable table is DS8 below, don't duplicate.
     U17 (advisory): fold in ONE sentence naming a real system where this is a workhorse (cross-link system-design where natural). -->

## Comparison

<!-- DS8 (gated): a TABLE — rows = this structure + real rivals (segment tree, sorted intervals + binary search), columns = key ops' time/space + the distinguishing trade-off. Scannable. -->

## Variants

<!-- DS5 (advisory): ≥1 real variant (e.g. centered interval tree, segment tree for intervals). -->

## Traversal & invariant

<!-- FB: Tree/heap family block — the augmented BST invariant: each node stores max endpoint in its subtree; search prunes branches where max_endpoint < query_start. -->
...
<!-- DS7: ≥2 interview traps, including ≥1 CP-flavored where relevant. -->
