...
<!-- PA2: the mechanic + a REAL diagram (BFS/DFS on 2D grid, direction vectors, boundary checks). -->

## Skeleton

<!-- PA3: reusable CLRS pseudocode + a Python template with a real signature, no syntax errors, and a `# your logic here` marker. Cover BFS variant (flood fill, shortest path) and DFS variant (connected components, island counting). -->
...
<!-- PA10 (gated): input size → reach for this pattern (or not). State when the constraint pushes you OFF this pattern. -->

## Variations

<!-- PA5 (advisory): common twists (spiral traversal, diagonal traversal, BFS for shortest path in grid, DFS for island count). -->

## CP-primitives

<!-- PA11 (gated): ≥2 contest-flavored tricks/variants (e.g. 0-1 BFS on grid, multi-source BFS, grid as implicit graph). Each with one-line "why for CP". -->
...
<!-- PA9 (advisory): the 2–3 sentences said out loud on spotting the pattern. -->
