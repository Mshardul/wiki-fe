...
Don't hand-roll KMP when a **library substring search** suffices - Python's `str.find` / `in`, C++ `std::string::find`, Java `indexOf` are well-optimized (often Boyer–Moore-flavored) and faster to type under interview pressure; use KMP only when asked to implement it or when you need its byproducts. For **multiple patterns** searched against one text, KMP per pattern is wasteful - reach for **[Aho-Corasick](./aho-corasick.md)** (a trie with KMP-style failure links). For **fast probabilistic** matching, equality of many substrings, or 2-D pattern search, prefer **Rabin–Karp** rolling hashes. And for typical English text where the pattern rarely self-overlaps, **Boyer–Moore** can beat KMP in practice by skipping ahead via the bad-character rule (sublinear on average), though its worst case isn't better.
