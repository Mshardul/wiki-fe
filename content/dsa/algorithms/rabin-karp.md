...
| `k` patterns, sum of lengths ≤ 10⁵               | O(n + k·m) or better | Rabin-Karp shines: hash all `k` patterns into a set; each window does O(1) set membership lookup → O(n + k·m) total. **[Aho-Corasick](./aho-corasick.md)** achieves O(n + Σm + matches) deterministically but requires building a trie. |
