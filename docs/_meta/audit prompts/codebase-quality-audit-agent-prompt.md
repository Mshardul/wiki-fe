...
- **Switch on `ApiError.code`, never on `message` text** (CONVENTIONS.md → Errors & API). Flag any caller that branches on human-readable message strings or invents FE-local code strings not in the cross-repo contract (`docs/_meta/decisions/auth.md` + BE).
