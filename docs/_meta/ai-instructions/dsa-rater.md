...
| U9  | Prerequisites format | adv  | `[Title](./path.md) [tier]` per entry - name and tier only, no reason sentence. Tier is exactly `[Must read]` or `[Should read]` - a third tier (`[Recommended]` etc.) is a format violation. Also judge tier **correctness**, not just presence: an entry marked `[Must read]` that the article doesn't actually depend on (or a `[Should read]` that's actually load-bearing) should be flagged in the NOTE even though it doesn't fail the format check itself.                                                                              |
...
- **CP-primitives (PA11 / CP) vs U6 - do not conflate.** CP-primitives = contest-flavored techniques the pattern/structure unlocks, each a tiny diagram/snippet + one-line "why for CP" - often a technique applying to a category of problem, not one named instance (meet-in-the-middle applies to any subset-sum-shaped problem). U6 (practice problems) = full worked entries, each a specific named problem with statement + worked examples + constraints + code + complexity + duplicate list. If a CP-primitives entry is written as a full worked-problem entry (statement/examples/constraints), score CP-primitives down for being over-written - it should look like a technique reference, not a problem writeup. If a U6 entry skips the statement/examples/constraints and reads like a bare technique snippet, score U6 down. The two sections must look visually different on the page.
...
| Gate            | Params                                                                                                                                                                                                                                                                                                                                                                    |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **gated**       | U1–U8, U11, U12 · all DS except DS5, DS9, DS9a · DS8 · all AL except AL8 · AL9, AL10 · PA1–PA4, PA7, PA8, PA10, PA11 · FB · V1, V2, V6, V9                                                                                                                                                                                                                               |
...
| V6  | Invariant inductive proof       | Check the stated loop/recurrence invariant actually has a base case AND an inductive step shown, not just asserted.                                        | AL3, DS9a - **n/a for Patterns** (no PA correctness/invariant param exists) | gate |
