...
Also test **orientation change**: for the 375px and 393px profiles, resize to the landscape dimensions too (e.g. 667×375, 852×393) and re-check modals, TOC drawer, and sticky header. Landscape phone height is the classic case where fixed-height/sticky elements eat the whole viewport. Also rotate the 768px tablet profile (768×1024 → 1024×768) — a real, common case (iPad rotation) with its own layout risks (TOC sidebar/content-width behavior at 1024px width is otherwise untested by this matrix); 360px stays portrait-only, landscape at that width is a rare device case not worth the pass.
...
- **Do not run any tests** — full suite or individual files. Assume the existing mobile-related tests are correct and passing; this audit is a fresh manual pass, not a test run.
