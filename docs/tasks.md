...
| WIKI-066 | 2026-04-22 | Add `defer` to CDN script tags                     | `showdown`, `highlight.js`, `mermaid` script tags are synchronous — block HTML parsing. `mermaid` alone is ~2 MB. Add `defer` to all three; execution order preserved.                                                     | Backlog   | —          |                                                                                                        |
| WIKI-070 | 2026-04-22 | Specific errors on content load failure            | Network error and 404 both show "Failed to load content." Distinguish in catch: HTTP 404 → "Article not found", network error → "Check your connection".                                                                   | Backlog   | —          |                                                                                                        |
| WIKI-072 | 2026-04-22 | Disable stub cards by default; enable after fetch  | Cards render clickable; `populateIndexReadTimes` disables stubs async — race lets users click through to dead-end. Add `index-card--unavailable` in template; remove when fetch confirms non-stub.                         | Backlog   | —          |                                                                                                        |
| WIKI-073 | 2026-04-22 | Add home fallback to 404 `history.back()`          | Bookmarked stub URL in fresh tab: `history.length === 1`, so `back()` exits the site. Fallback: `if (history.length > 1) history.back(); else location.href = './index.html'`.                                             | Backlog   | —          |                                                                                                        |
